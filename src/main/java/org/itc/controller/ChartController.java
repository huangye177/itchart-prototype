...
public class ChartController
{
    ...
    private final String cssChunckStart = "<defs> <style type=\"text/css\"><![CDATA[ ";

    private final String cssChunckEnd = " ]]></style> </defs>";
     ...
     * @param request
     */
    @RequestMapping(value = "/chart/convert2pdf", method = RequestMethod.POST)
    public synchronized void convertSVGtoPDF(HttpServletRequest request, HttpServletResponse response)
    {
        String rawdata = "";
        String rawcss = "";
        String data = "";

        InputStream inputStreamForPDF = null;
        String cssChunk = "";

        try
        {
            // get data
            rawdata = request.getParameter("svgxml_data");
            rawcss = request.getParameter("svgxml_css");
            ...
            cssChunk = cssChunckStart + rawcss + cssChunckEnd;
            data = data_start + cssChunk + data_end;

            // prepare input stream
            // inputStreamForPDF = new ByteArrayInputStream(rawdata.getBytes());
            inputStreamForPDF = new ByteArrayInputStream(data.getBytes());

            // convert input stream into output stream of the
            // HttpServletResponse
            SvgUtil.convert2Pdf(inputStreamForPDF, response.getOutputStream());
